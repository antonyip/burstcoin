...
final class TransactionProcessorImpl implements TransactionProcessor
{
    ...
    private static final int rebroadcastAfter = Nxt.getIntProperty("burst.rebroadcastAfter") != 0
            ? Nxt.getIntProperty("burst.rebroadcastAfter") : 4;
    private static final int rebroadcastEvery = Nxt.getIntProperty("burst.rebroadcastEvery") != 0
            ? Nxt.getIntProperty("burst.rebroadcastEvery") : 2;
    ...
    final DbKey.LongKeyFactory<TransactionImpl> unconfirmedTransactionDbKeyFactory = new DbKey.LongKeyFactory<TransactionImpl>(
            "id")
    {
    ...
    private final EntityDbTable<TransactionImpl> unconfirmedTransactionTable = new EntityDbTable<TransactionImpl>(
    ...
    private final Set<TransactionImpl> nonBroadcastedTransactions = Collections
            .newSetFromMap(new ConcurrentHashMap<TransactionImpl, Boolean>());
        ...
        public void run()
        {
            ...
            try
            {
                try
                {
                    ...
                    try (DbIterator<TransactionImpl> iterator = unconfirmedTransactionTable.getManyBy(expiredClause, 0,
                            -1, ""))
                    {
                    ...
                    synchronized (BlockchainImpl.getInstance())
                    {
                        if (lostTransactions.size() > 0)
                        {
                            ...
                            if (enableTransactionRebroadcasting
                                    && Nxt.getEpochTime() - Nxt.getBlockchain().getLastBlock().getTimestamp() < 4 * 60)
                            {
                                ...
                                for (Transaction lost : reAdded)
                                {
                                    if (lostTransactionHeights.containsKey(lost.getId()))
                                    {
                                        ...
                                        if (Nxt.getBlockchain().getHeight() - addedHeight >= rebroadcastAfter
                                                && (Nxt.getBlockchain().getHeight() - addedHeight - rebroadcastAfter)
                                                        % rebroadcastEvery == 0)
                                        {
            ...
            short deadline, Attachment attachment)
    {
        ...
        TransactionImpl.BuilderImpl builder = new TransactionImpl.BuilderImpl(version, senderPublicKey, amountNQT,
                feeNQT, timestamp, deadline, (Attachment.AbstractAttachment) attachment);
        if (version > 0)
        {
    ...
    private void processPeerTransactions(JSONArray transactionsData) throws NxtException.ValidationException
    {
        ...
        for (Object transactionData : transactionsData)
        {
            try
            {
                ...
                {
                    /*
                     * if(Nxt.getBlockchain().getHeight() >=
                     * Constants.EC_CHANGE_BLOCK_1) {
                     * throw new NxtException.
                     * NotValidException("Transaction from wrong fork");
                     * }
                     */
                    continue;
                }
            ...
            catch (NxtException.NotValidException e)
            {
                Logger.logDebugMessage(
                        "Invalid transaction from peer: " + ((JSONObject) transactionData).toJSONString());
                throw e;
            }
        }
            ...
            try
            {
                ...
                synchronized (BlockchainImpl.getInstance())
                {
                    try
                    {
                        ...
                        if (TransactionDb.hasTransaction(transaction.getId())
                                || unconfirmedTransactionTable.get(transaction.getDbKey()) != null)
                        {
                            continue;
                        }
                        ...
                        if (!(transaction.verifySignature() && transaction.verifyPublicKey()))
                        {
                            ...
                            {
                                Logger.logDebugMessage("Transaction " + transaction.getJSONObject().toJSONString()
                                        + " failed to verify");
                            }
                            continue;
                        }
