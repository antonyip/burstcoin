...
public final class DebugTrace
{
    ...
    { "height", "event", "account", "asset", "balance", "unconfirmed balance", "asset balance",
            "unconfirmed asset balance", "transaction amount", "transaction fee", "generation fee", "effective balance",
            "order", "order price", "order quantity", "order cost", "trade price", "trade quantity", "trade cost",
            "asset quantity", "transaction", "lessee", "lessor guaranteed balance", "purchase", "purchase price",
            "purchase quantity", "purchase cost", "discount", "refund", "sender", "recipient", "block", "timestamp" };
    ...
    private boolean include(Attachment attachment)
    {
        ...
        {
            long sellerId = DigitalGoodsStore.getGoods(((Attachment.DigitalGoodsPurchase) attachment).getGoodsId())
                    .getSellerId();
            return include(sellerId);
        }
        else if (attachment instanceof Attachment.DigitalGoodsDelivery)
        {
            long buyerId = DigitalGoodsStore.getPurchase(((Attachment.DigitalGoodsDelivery) attachment).getPurchaseId())
                    .getBuyerId();
            return include(buyerId);
        }
        else if (attachment instanceof Attachment.DigitalGoodsRefund)
        {
            long buyerId = DigitalGoodsStore.getPurchase(((Attachment.DigitalGoodsRefund) attachment).getPurchaseId())
                    .getBuyerId();
            return include(buyerId);
        }
        return false;
    ...
    private Map<String, String> getValues(long accountId, Block block)
    {
        ...
        // String.valueOf(Account.getAccount(accountId).getEffectiveBalanceNXT()));
    ...
    /*
     * private Map<String,String> getValues(long accountId, Account.AccountLease
     * accountLease, boolean start) {
     * Map<String,String> map = new HashMap<>();
     * map.put("account", Convert.toUnsignedLong(accountId));
     * map.put("event", start ? "lease begin" : "lease end");
     * map.put("timestamp",
     * String.valueOf(Nxt.getBlockchain().getLastBlock().getTimestamp()));
     * map.put("height", String.valueOf(Nxt.getBlockchain().getHeight()));
