...
{
    public static final int BLOCKCACHEMB = Nxt.getIntProperty("burst.blockCacheMB") == 0 ? 40
            : Nxt.getIntProperty("blockCacheMB");
    public static boolean oclVerify = Nxt.getBooleanProperty("burst.oclVerify");
    public static final int oclThreshold = Nxt.getIntProperty("burst.oclThreshold") == 0 ? 50
            : Nxt.getIntProperty("burst.oclThreshold");
    public static final int oclWaitThreshold = Nxt.getIntProperty("burst.oclWaitThreshold") == 0 ? 2000
            : Nxt.getIntProperty("burst.oclWaitThreshold");
        ...
        public void run()
        {
            for (;;)
            {
                ...
                else
                {
                    ...
                    {
                        if (BlockchainProcessorImpl.unverified.size() == 0) return;
            ...
            while (true)
            {
                synchronized (blockchain)
                {
                    for (;;)
                    {
                        ...
                        synchronized (BlockchainProcessorImpl.blockCache)
                        {
                            if (!BlockchainProcessorImpl.reverseCache.containsKey(lastId)) break;
                        ...
                        synchronized (BlockchainProcessorImpl.blockCache)
                        {
                            ...
                            { // make sure it wasn't already removed(ex failed
                              // preValidate) to avoid double subtracting from
                              // blockCacheSize
                            ...
                            /*
                             * if (blockchain.getLastBlock().getId() ==
    ...
    private void blacklistClean(BlockImpl block, Exception e)
    {
        ...
        synchronized (BlockchainProcessorImpl.blockCache)
        {
            ...
            lastDownloaded = lastBlock.getHeight() >= block.getHeight() ? lastBlock.getId()
                    : block.getPreviousBlockId();
            blockCache.notify();
        }
    }
    ...
    private final Runnable getMoreBlocksThread = new Runnable()
    {
        ...
        public void run()
        {
            try
            {
                try
                {
                    ...
                    if (blockCacheSize > BLOCKCACHEMB * 1024 * 1024) return;
                    ...
                    if (commonBlock.getHeight() < blockchain.getLastBlock().getHeight())
                    { // fork point
                    ...
                    int requestCount = 0;
                    outer: while (forkBlocks.size() < 1440 && requestCount++ < 10
                            && ((blockCacheSize < BLOCKCACHEMB * 1024 * 1024) || forkBlocks.size() > 0))
                    { // fork decision could be wrong if cut off so ignore cache
                      // size for forks
                        // Logger.logMessage("Downloading " +
                        // String.valueOf(currentBlockId));
                        ...
                        synchronized (BlockchainProcessorImpl.blockCache)
                        {
                            for (Object o : nextBlocks)
                            {
                                ...
                                try
                                {
                                    ...
                                    if (block.getPreviousBlockId() != currentBlockId)
                                    { // ensure peer isn't cluttering cache with
                                      // unrequested stuff
                                    ...
                                    if (reverseCache.containsKey(block.getPreviousBlockId()))
                                    {
                                        ...
                                        {
                                            Logger.logMessage(
                                                    "Aborting getMoreBlocks. Conflicting fork already in queue.");
                                            return;
                                        }
                                    }
                                    ...
                                    else
                                    {
                                        ...
                                        if (prevBlock == null)
                                        {
                                            // We may be on a fork: Add all
                                            // other Blocks to forkQueue:
                                            ...
                                            {
                                                // Logger.logMessage("Previous
                                                // Block with ID " +
                                                // String.valueOf(prevId) + "
                                                // not found. Blacklisting
                                                // ...");
                                                // peer.blacklist();

                                                // We've already verified that
                                                // we asked for a block with
                                                // that prevBlock,
                                                // so not the peer's fault if we
                                                // changed what we wanted before
                                                // we receive the data.
                                                return;
                                            }
                                        }
                                    ...
                                    // Logger.logMessage("Block Height " +
                                    // String.valueOf(block.getHeight()) + " ID
                                    // " + String.valueOf(block.getId()));

                                    if (forkBlocks.size() == 0)
                                    { // keep old cache separate from fork.
                                      // blindly adding to reverseCache could
                                      // cause it to end up in an uncleanable
                                      // state
                                        // Add to Blockcache
        ...
        private long getCommonMilestoneBlockId(Peer peer)
        {
            ...
            while (true)
            {
                ...
                {
                    Logger.logDebugMessage("Obsolete or rogue peer " + peer.getPeerAddress()
        ...
        private JSONArray getNextBlocks(Peer peer, long curBlockId)
        {
            ...
            if (nextBlocks.size() > 1440)
            {
                Logger.logDebugMessage(
                        "Obsolete or rogue peer " + peer.getPeerAddress() + " sends too many nextBlocks, blacklisting");
                peer.blacklist();
                return null;
            }
        ...
        private void processFork(Peer peer, final List<BlockImpl> forkBlocks, final Block commonBlock)
        {
            ...
            synchronized (blockCache)
            { // cache may no longer correspond with current chain, so dump it
                blockCache.clear();
                reverseCache.clear();
                unverified.clear();
    ...
    public void processPeerBlock(JSONObject request) throws NxtException
    {
        ...
        synchronized (blockchain)
        {
            synchronized (blockCache)
            {
                ...
                if (blockCache.containsKey(block.getPreviousBlockId())
                        && !(reverseCache.containsKey(block.getPreviousBlockId())))
                {
                ...
                if (block.getPreviousBlockId() == prevBlock.getId())
                {
                    if (reverseCache.containsKey(prevBlock.getId()))
                    {
                        ...
                        if (existingId != block.getId())
                        {
                            Logger.logMessage("Ignoring peer broadcast block with ID "
                                    + Convert.toUnsignedLong(block.getId()) + ". Conflicting block "
                                    + Convert.toUnsignedLong(existingId) + " already exists in queue");
                        }
                        else
                        {
                            Logger.logDebugMessage("Ignoring peer broadcast block with ID "
                                    + Convert.toUnsignedLong(block.getId()) + ". Already exists in queue");
                        }
                    }
                    else
                    {
                        ...
                        block.setByteLength(blockSize);
                        blockCacheSize += block.getByteLength();
                        // do not add to unverified as it will be processed
                        // immediately
                ...
                {
                    Logger.logDebugMessage("Ignoring peer broadcast block with ID "
                            + Convert.toUnsignedLong(block.getId()) + ". Previous block "
                            + Convert.toUnsignedLong(block.getPreviousBlockId())
    ...
    private void pushBlock(final BlockImpl block) throws BlockNotAcceptedException
    {
        ...
        synchronized (blockchain)
        {
            ...
            try
            {
                ...
                if (block.getVersion() != 1 && !Arrays.equals(Crypto.sha256().digest(previousLastBlock.getBytes()),
                ...
                for (TransactionImpl transaction : block.getTransactions())
                {
                    ...
                    if (Nxt.getBlockchain().getHeight() >= Constants.AUTOMATED_TRANSACTION_BLOCK)
                    {
                        ...
                        {
                            Logger.logDebugMessage("Block " + block.getStringId() + " height "
            ...
            throws TransactionNotAcceptedException, BlockNotAcceptedException
    {
        ...
        catch (AT_Exception e)
        {
            throw new BlockNotAcceptedException(
                    "ats are not matching at block height " + Nxt.getBlockchain().getHeight());
        }
    ...
    int getBlockVersion(int previousBlockHeight)
    {
        return previousBlockHeight < Constants.TRANSPARENT_FORGING_BLOCK ? 1
                : previousBlockHeight < Constants.NQT_BLOCK ? 2 : 3;
    }
    ...
    void generateBlock(String secretPhrase, byte[] publicKey, Long nonce) throws BlockNotAcceptedException
    {
        ...
        try (FilteringIterator<TransactionImpl> transactions = new FilteringIterator<>(
                transactionProcessor.getAllUnconfirmedTransactions(), new FilteringIterator.Filter<TransactionImpl>()
                {
                    @Override
                ...
                && blockTransactions.size() <= Constants.MAX_NUMBER_OF_TRANSACTIONS)
        {
            ...
            for (TransactionImpl transaction : orderedUnconfirmedTransactions)
            {
                ...
                if (Nxt.getBlockchain().getHeight() >= Constants.AUTOMATED_TRANSACTION_BLOCK)
                {
                    if (!EconomicClustering.verifyFork(transaction))
                    {
                        Logger.logDebugMessage("Including transaction that was generated on a fork: "
        ...
        if (Subscription.isEnabled())
        {
            synchronized (blockchain)
            {
                ...
                try
                {
                    Db.beginTransaction();
                    transactionProcessor.requeueAllUnconfirmedTransactions();
                    // transactionProcessor.processTransactions(newTransactions,
                    // false);
        ...
        AT.clearPendingFees();
        AT.clearPendingTransactions();
        AT_Block atBlock = AT_Controller.getCurrentBlockATs(Constants.MAX_PAYLOAD_LENGTH - payloadLength,
        ...
        byte[] generationSignature = Nxt.getGenerator()
                .calculateGenerationSignature(previousBlock.getGenerationSignature(), previousBlock.getGeneratorId());
        ...
        try
        {
            ...
            block = new BlockImpl(getBlockVersion(previousBlock.getHeight()), blockTimestamp, previousBlock.getId(),
                    totalAmountNQT, totalFeeNQT, payloadLength, payloadHash, publicKey, generationSignature, null,
                    previousBlockHash, new ArrayList<>(blockTransactions), nonce, byteATs);
        ...
        try
        {
            ...
            Logger.logDebugMessage("Account " + Convert.toUnsignedLong(block.getGeneratorId()) + " generated block "
    ...
    public void scan(int height)
    {
        synchronized (blockchain)
        {
            ...
            {
                throw new IllegalArgumentException("Rollback height " + (height - 1)
                        + " exceeds current blockchain height of " + blockchainHeight);
            }
            if (height > 0 && height < getMinRollbackHeight())
            {
                Logger.logMessage("Rollback of more than " + Constants.MAX_ROLLBACK
                        + " blocks not supported, will do a full scan");
                height = 0;
            }
            if (height < 0)
            {
            ...
            try (Connection con = Db.beginTransaction();
                    PreparedStatement pstmt = con
                            .prepareStatement("SELECT * FROM block WHERE height >= ? ORDER BY db_id ASC"))
            {
                transactionProcessor.requeueAllUnconfirmedTransactions();
                Account.flushAccountTable();
                for (DerivedDbTable table : derivedTables)
                ...
                try (ResultSet rs = pstmt.executeQuery())
                {
                    ...
                    if (height == 0)
                    {
                        blockchain.setLastBlock(currentBlock); // special case
                                                               // to avoid no
                                                               // last block
                    ...
                    while (rs.next())
                    {
                        try
                        {
                            ...
                            if (currentBlock.getId() != currentBlockId)
                            {
                                ...
                                {
                                    Logger.logDebugMessage("Wrong genesis block id set. Should be: "
                            ...
                            if (validateAtScan && currentBlockId != Genesis.GENESIS_BLOCK_ID)
                            {
                                ...
                                JSONObject blockJSON = (JSONObject) JSONValue
                                ...
                                for (TransactionImpl transaction : currentBlock.getTransactions())
                                {
                                    ...
                                    if (transaction.getVersion() != transactionProcessor
                                            .getTransactionVersion(blockchain.getHeight()))
                                    {
                                        throw new NxtException.NotValidException("Invalid transaction version");
                                    }
                                    /*
                                     * if (!EconomicClustering.verifyFork(
                                     * transaction)) {
                                     * Logger.
                                     * logDebugMessage("Found transaction that was generated on a fork: "
                                     * + transaction.getStringId()
                                     * + " in block " +
                                     * currentBlock.getStringId() +
                                     * " at height " + currentBlock.getHeight()
                                     * + " ecBlockHeight " +
                                     * transaction.getECBlockHeight() +
                                     * " ecBlockId " +
                                     * Convert.toUnsignedLong(transaction.
                                     * getECBlockId()));
                                     * //throw new NxtException.
                                     * NotValidException("Invalid transaction fork"
                                     * );
                                     * }
                                     */
                                    transaction.validate();
                                    ...
                                    JSONObject transactionJSON = (JSONObject) JSONValue
                        ...
                        catch (NxtException | RuntimeException e)
                        {
                            ...
                            Logger.logDebugMessage("Applying block " + Convert.toUnsignedLong(currentBlockId)
