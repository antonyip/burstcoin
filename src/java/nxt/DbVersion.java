...
final class DbVersion
{
    ...
    private static void update(int nextUpdate)
    {
        switch (nextUpdate)
        {
            ...
            case 104:
                // apply("CREATE INDEX IF NOT EXISTS
                // account_current_lessee_id_leasing_height_idx ON account
                // (current_lessee_id, "
            ...
            case 107:
                ...
                // (db_id IDENTITY, account_id BIGINT NOT NULL, "
                // + "additions BIGINT NOT NULL, height INT NOT NULL)");
                apply(null);
            case 108:
                // apply("CREATE UNIQUE INDEX IF NOT EXISTS
                // account_guaranteed_balance_id_height_idx ON
            ...
            default:
                throw new RuntimeException(
                        "Database inconsistent with code, probably trying to run older code on newer database");
        }
    }
