...
final class PeerImpl implements Peer
{
    ...
    void setVersion(String version)
    {
        ...
        if (Nxt.APPLICATION.equals(application) && version != null)
        {
            ...
            if (isOldVersion)
            {
                // Logger.logDebugMessage("Blacklisting %s version %s",
                // peerAddress, version);
            }
        }
    }
    ...
    void connect()
    {
        ...
        if (response != null)
        {
            ...
            if (announcedAddress == null)
            {
                ...
                // Logger.logDebugMessage("Connected to peer without announced
                // address, setting to " + peerAddress);
            }
    ...
    boolean analyzeHallmark(String address, final String hallmarkString)
    {
        ...
        try
        {
            ...
            if (!hallmark.isValid() || !(hallmark.getHost().equals(host)
                    || InetAddress.getByName(host).equals(InetAddress.getByName(hallmark.getHost()))))
            {
                // Logger.logDebugMessage("Invalid hallmark for " + host + ",
                // hallmark host is " + hallmark.getHost());
                return false;
            }
