...
final class OCLPoC
{
    ...
    private static final int hashesPerEnqueue = Nxt.getIntProperty("burst.oclHashesPerEnqueue") == 0 ? 1000
            : Nxt.getIntProperty("burst.oclHashesPerEnqueue");
    private static final int memPercent = Nxt.getIntProperty("burst.oclMemPercent") == 0 ? DEFAULT_MEM_PERCENT
            : Nxt.getIntProperty("burst.oclMemPercent");
    ...
    static
    {
        try
        {
            ...
            ctx = clCreateContext(ctxProps, 1, new cl_device_id[]
            ...
            program = clCreateProgramWithSource(ctx, 1, new String[]
            { source }, null, null);
    ...
    public static void validatePoC(Collection<BlockImpl> blocks)
    {
        try
        {
            ...
            // System.out.println("ocl blocks: " + blocks.size() + " jobSize: "
            // + jobSize);
            ...
            synchronized (oclLock)
            {
                ...
                try
                {
                    idMem = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 8 * blocks.size(),
                            Pointer.to(ids), null);
                    nonceMem = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 8 * blocks.size(),
                            Pointer.to(nonces), null);
                    bufferMem = clCreateBuffer(ctx, CL_MEM_READ_WRITE, (MiningPlot.PLOT_SIZE + 16) * blocks.size(),
                            null, null);
                    scoopNumMem = clCreateBuffer(ctx, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, 4 * blocks.size(),
                            Pointer.to(scoopNums), null);
                    scoopOutMem = clCreateBuffer(ctx, CL_MEM_READ_WRITE, MiningPlot.SCOOP_SIZE * blocks.size(), null,
                            null);

                    int[] totalSize = new int[]
                    { blocks.size() };
                    ...
                    while (c < 8192)
                    {
                        ...
                        clEnqueueNDRangeKernel(queue, genKernel, 1, null, new long[]
                        { jobSize }, new long[]
                        { maxGroupItems }, 0, null, null);
                    ...
                    clEnqueueNDRangeKernel(queue, getKernel, 1, null, new long[]
                    { jobSize }, new long[]
                    { maxGroupItems }, 0, null, null);

                    clEnqueueReadBuffer(queue, scoopOutMem, true, 0, MiningPlot.SCOOP_SIZE * blocks.size(),
        ...
        {
            throw new OCLCheckerException("OpenCL error", e); // intentionally
                                                              // leave out of
                                                              // unverified
                                                              // cache. It won't
                                                              // slow it that
    ...
    { // idk if the kernel works on big endian, but I'm guessing not and I don't
      // have the hardware to find out
    ...
    static private AutoChooseResult autoChooseDevice()
    {
        ...
        for (int pfi = 0; pfi < platforms.length; pfi++)
        {
            ...
            clGetPlatformInfo(platforms[pfi], CL_PLATFORM_NAME, platformNameChars.length, Pointer.to(platformNameChars),
                    null);
            ...
            for (int dvi = 0; dvi < devices.length; dvi++)
            {
                ...
                long maxItemsAtOnce = Math.min(calculateMaxItemsByMem(devices[dvi]),
                        getComputeUnits(devices[dvi]) * 256);
